<template>
<!-- 	<view class="contain">
	<view class="navbar"><image src="@/static/assets/image/back.png" style='height: 4vh;width: 8vw;position: absolute;left: 2vw;'  mode="" @click="gotoIndex()"></image>深度详情</view>
	</view> -->
	<view class="page">
		<echart class="echarts" :style="{width:'100vw',height:'35vh',}" :option="Options"></echart>
		<view class="detail">
			<view class="card">
				<view class="temp">{{Lowest}} m</view>
				<view class="name">最低值</view>
			</view>
			<view class="card">
				<view class="temp">{{Average}} m</view>
				<view class="name">当前值</view>
			</view>
			<view class="card-end">
				<view class="temp">{{Highest}} m</view>
				<view class="name">最高值</view>
			</view>
		</view>
		<view class="tips">* 适宜鳗鱼养殖的水深应在1.5 m ~ 2.5 m </view>
	</view>
</template>
		...
		methods: {
			...
			GetHighest(){
				uni.request({
					...
					success: (res) => {
						var High
						High = res.data[0].Depth
						for(var i = 0;i < 5;i++){
							if(res.data[i].Depth > High){
								High = res.data[i].Depth
							}
						}
						this.Highest = High
					}
			...
			GetLowest(){
				uni.request({
					...
					success: (res) => {
						var Low
						Low = res.data[0].Depth
						for(var i = 0;i < 5;i++){
							if(res.data[i].Depth < Low){
								Low = res.data[i].Depth
							}
						}
						this.Lowest = Low
					}
			...
			GetDataFromMySqlData: function() {
				...
				uni.request({
					...
					success: (res) => {
						that.data = [res.data[4].Depth,res.data[3].Depth,res.data[2].Depth,res.data[1].Depth,res.data[0].Depth]
